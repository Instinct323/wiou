...
def simulate_exp(loss_fcn, lr=.01, max_iter=120,
    ...
    if plot_points:
        ...
        for l, t, r, b in target:
            ...
            fig.add_patch(rect)
        plt.show()
    # Visualize the gradient generated by the anchor boxes
    # anchor.requires_grad = True
    # IoU_Cal.iou_mean = 0.25
    # (loss_fcn(anchor, target).mean()).backward()
    # grad = torch.square(anchor.grad).mean(dim=(1, 2, 3)).sqrt()
    # fig = plt.subplot(projection='3d')
    # plt.title(loss_fcn.__name__)
    # fig.set_xlabel('x')
    # fig.set_ylabel('y')
    # fig.set_zlabel('grad')
    # fig.view_init(40, 30)
    # fig.scatter(x, y, grad, cmap=plt.get_cmap('rainbow'), c=(grad - grad.min()) / (grad.max() - grad.min()))
    # if file_mode:
    #     folder = Path(file_mode).parent
    #     folder.mkdir(exist_ok=True)
    #     plt.savefig(file_mode % loss_fcn.__name__)
    #     plt.close()
    # else:
    #     plt.show()
    # IoU_Cal.iou_mean = 1.
...
if __name__ == '__main__':
    ...
    command = [lambda: plot_loss([f.WIoU, f.SIoU, f.CIoU, f.DIoU, f.GIoU], n_points=None, major_cases=False),
               lambda: plot_loss([f.WIoU, f.SIoU], n_points=500, major_cases=False),
